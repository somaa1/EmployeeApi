        ...
        try{
            ...
            function insertNewEmployee($username, $password, $email, $image) {
                ...
                function updateEmployee($id, $username, $password, $email, $image) {
                    try {
                        $sql = "UPDATE employees SET username='$username', password='$password', email='$email', image='$image' WHERE id=$id";
                        $result = $this->conn->query($sql);
            
                        if ($result == true) {
                            $this->createResponse(true, 1, "Employee updated successfully");
                        } else {
                            $this->createResponse(false, 0, "Failed to update employee");
                        }
                    } catch (Exception $error) {
